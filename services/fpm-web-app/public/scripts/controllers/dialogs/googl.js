...
angular.module('fpmApp')
  .controller('GooglCtrl', ['$rootScope', '$scope', '$window', '$state', '$mdDialog', 'Google', 'config', '_', 'urls', function($rootScope, $scope, $window, $state, $mdDialog, Google, config, _, urls) {
    ...
    $scope.connect = function(connectToAllAccounts) {
      $window.location.href = urls.urlParams(config.api.url+config.api.ops.googl[connectToAllAccounts ? 'authProfile' : 'authAccount'], {

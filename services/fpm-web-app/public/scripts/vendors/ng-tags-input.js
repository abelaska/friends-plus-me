...
tagsInput.directive('tagsInput', ["$timeout","$document","tagsInputConfig", function($timeout, $document, tagsInputConfig) {
    ...
    return {
        ...
        controller: ["$scope","$attrs","$element", function($scope, $attrs, $element, cfpLoadingBar) {
            ...
            this.registerAutocomplete = function() {
                ...
                return {
                    ...
                    autocompleteLoadStarted: function() {
                        $scope.loadInProgress = true;
                    },
                    autocompleteLoadStopped: function() {
...
tagsInput.directive('autoComplete', ["$document","$timeout","$sce","tagsInputConfig", function($document, $timeout, $sce, tagsInputConfig) {
    function SuggestionList(loadFn, options) {
        ...
        self.load = function(query, tags, onInit, onDone, filter) {
            $timeout.cancel(debouncedLoadId);
            debouncedLoadId = $timeout(function() {
                ...
                var promise = loadFn({ $query: query, $filter: filter });
                lastPromise = promise;
                ...
                promise.then(function(items) {
                    ...
                    // zakomentovano aby se neodmazavali uzivatele se stejnym jmenem
    ...
    return {
        ...
        link: function(scope, element, attrs, tagsInputCtrl) {
            ...
            scope.addSuggestion = function() {
                var added = false,
                    selected = suggestionList.selected;
                if (selected) {
                    if (selected.type === 'community') {
                        suggestionList.load('', tagsInput.getTags(), function() {
                            tagsInput.autocompleteLoadStarted();
                        }, function() {
                            tagsInput.autocompleteLoadStopped();
                        }, {type: 'community', community: selected});
                    } else {
                        if (selected.type === 'community-thread') {
                            var communityThread = selected;
                            selected = communityThread.community;
                            selected.thread = communityThread;
                        }

                        tagsInput.addTag(selected);
            ...
            tagsInput
                ...
                .on('input-change', function(value) {
                    if (shouldLoadSuggestions(value)) {
                        suggestionList.load(value, tagsInput.getTags(), function() {
                            tagsInput.autocompleteLoadStarted();
                        }, function() {
                            tagsInput.autocompleteLoadStopped();
                        });
                ...
                .on('input-keydown', function(e) {
                    ...
                    if (suggestionList.visible) {
                        ...
                        if (key === KEYS.down && scope.options.loadOnDownArrow) {
                            suggestionList.load(tagsInput.getCurrentTagText(), tagsInput.getTags(), function() {
                                tagsInput.autocompleteLoadStarted();
                            }, function() {
                                tagsInput.autocompleteLoadStopped();
                            });

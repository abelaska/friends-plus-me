...
const PostConvertorTwitter = require('../PostConvertorTwitter');
...
test('should convert post with @mention', t => {
  const post = new Post({
    ...
    processor: 'post:twitter:profile',
    repost: {
      is: false
    },
    reshare: {
  ...
  const profile = new Profile({
    accounts: [{ account: Types.account.profile.code }]
  });
  const account = profile.accounts[0];
  const conv = new PostConvertorTwitter(post, profile, account, (url, lsCallback) => lsCallback(null, url, 'none'));
...
test('should convert post with unicode art', t => {
  ...
  conv.convert((err, req) => {
    t.falsy(err);
    t.truthy(req);
    t.is(req.status, '@knowntocollapse\n🌞 ⓗⓐⓟⓟⓨ ⓑⓘⓡⓣⓗⓓⓐⓨ 🌝\n(¯`·.¸¸.·´¯`·.¸¸.-> ⓚⓔⓥⓘⓝ <-.¸¸.·´¯`·.¸¸.·´¯)');
  });
});
